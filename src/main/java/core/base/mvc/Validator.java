...
/**
 * Mostly used for validation of bean; 
 * @author Jason
 *
 * 2015年9月9日
 */
public class Validator {
    ...
    /**
     * Initialize validator by target Object to validation
     * 
     * @param target
     */
    public Validator(Object target) {
        if(target == null) throw new ValidateException("对象为空");
    ...
    /**
     * Choose current property for validation,get value of which
     * 
     * @param name
     * @return
     */
    public Validator forProperty(String name) {
        System.out.println("lookupVariable:   " + sec.lookupVariable(name));
        this.name = name;
    ...
    /**
     * Exit validation of Object
     */
    public Validator checkOut() {
        if (errors.size() > 0) throw new ValidateException(this.errors);
        return this;
    }

    /**
     * Check if error about current property is exist.
     * 
     * @return
     */
    ...
    public Validator notNull() {
        if (value == null || !error()) errors.put(name, "不可为空");
        return this;
    }
